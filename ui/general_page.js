...
class LiveEngineGeneralPage extends Adw.PreferencesPage {
    _init(settings) {
        super._init({
            title: 'General',
        ...
        this.add(this._preview.group);

        const group = new Adw.PreferencesGroup();
        group.add(this._buildEnabledRow());
    ...
    _buildEnabledRow() {
        const row = new Adw.SwitchRow({
            ...
            subtitle: 'Start or stop the live wallpaper',
        });
        this._settings.bind(Keys.ENABLED, row, 'active', Gio.SettingsBindFlags.DEFAULT);
        return row;
    }
        ...
        dialog.open(window, null, (d, result) => {
            try {
                ...
                if (file) {
                    ...
                    this._preview.video.set_filename(file.get_path());
                } else {
                    row.subtitle = 'None';
                    this._settings.set_string(Keys.VIDEO_PATH, '');
                    this._preview.video.set_file(null);
                }
            } catch (e) {
